...
<template>
    <div>
        ...
        <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" class="mb-8 flex items-center justify-between bg-red-400 rounded w-full">
            <div class="flex items-center">
                ...
                <div v-else class="py-4 text-white text-sm font-medium">
                    <span>Hay {{ Object.keys($page.props.errors).length }} error{{ Object.keys($page.props.errors).length > 1 ? 'es' : '' }} en el formulario.</span>
                </div>
            </div>
            <button type="button" class="group mr-2 p-2" @click="show = false">
